...
Producer::OnInterest(shared_ptr<const Interest> interest)
{
  ...
  static const std::string content = "BJTUNGIRC";
  ...
  std::cout<<"node(" << GetNode()->GetId() << ") responding with Data's Content(length): " << data->getContent().size()<< std::endl;
 
  Name tempdata = data->getFullName();
  data->setDigest(tempdata.wireEncode());
  std::cout<<"Fulldata" << tempdata << std::endl;
