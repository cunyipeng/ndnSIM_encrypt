...
Data::wireDecode(const Block& wire)
{
  ...
  // Digest
  m_fullName1 = m_wire.get(tlv::Digest);
}
...
Data::getFullName() const
{
  if (m_fullName.empty()) {
    ...
    m_fullName.appendImplicitSha256Digest(crypto::sha256(m_content.wire(), m_content.size()));
    }
    return m_fullName;
}

const Name&
Data::VerifyDigest() const
{
   m_fullName2.appendImplicitSha256Digest(crypto::sha256(m_content.wire(), m_content.size()));
   if(m_fullName1 == m_fullName2.wireEncode())
       std::cout << "Verify successfully! The Content isn't modified." << std::endl;
   else
       std::cout << "Verify failure! The Content has been modified." << std::endl;
   return m_fullName2;
}
...
operator<<(std::ostream& os, const Data& data)
{
  ...
  os << "Content's length: " << data.getContent().size()<< "\n";
